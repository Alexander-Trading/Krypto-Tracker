...
async def history_candles_bulk(inst_id, start_day, end_day, bar="1D"):
    """Wie eur_rate_on()/historic_instrument_price(), aber holt einen ganzen
    Zeitraum in wenigen Anfragen statt einer pro Tag - OKX liefert bis zu 100
    Kerzen je Aufruf. Wird rueckwaerts paginiert (OKX' "after" = "aeltere
    Kerzen als dieser Zeitstempel", neueste zuerst in der Antwort).
    Gibt {day: close} zurueck, day als 'YYYY-MM-DD'. Bricht sauber ab, wenn
    OKX fuer einen Teil des Zeitraums nichts hat (z.B. Kontrakt noch nicht
    gelistet) - liefert dann einfach weniger Tage zurueck."""
    from datetime import datetime, timezone, timedelta
    start_ms = int(datetime.fromisoformat(start_day)
                   .replace(tzinfo=timezone.utc).timestamp() * 1000)
    end_ms = int((datetime.fromisoformat(end_day).replace(tzinfo=timezone.utc)
                 + timedelta(days=1)).timestamp() * 1000)

    out = {}
    cursor = end_ms
    for _ in range(50):   # Sicherheitsgrenze gegen Endlosschleifen
        if cursor <= start_ms:
            break
        try:
            data = await _get("/api/v5/market/history-candles", instId=inst_id,
                              bar=bar, after=str(cursor), limit="100")
        except MarketError:
            break
        if not data:
            break
        ...
        for row in data:
            ...
            if close != 0:
                day = datetime.fromtimestamp(
                    ts_ms / 1000, tz=timezone.utc).date().isoformat()
                out[day] = close
        if oldest_ts is None or oldest_ts >= cursor:
            break  # kein Fortschritt mehr moeglich
        cursor = oldest_ts
    return out


async def historic_rates_bulk(asset, start_day, end_day):
    """Wie eur_rate_on(), aber fuer einen ganzen Zeitraum auf einmal - siehe
    history_candles_bulk(). Probiert Asset-EUR, dann EUR-Asset (invertiert),
    genau wie eur_rate_on()."""
    if asset == "EUR":
        return {}
    for inst, invert in ((f"{asset}-EUR", False), (f"EUR-{asset}", True)):
        prices = await history_candles_bulk(inst, start_day, end_day)
        if prices:
            if invert:
                return {d: (Decimal(1) / p) for d, p in prices.items() if p != 0}
            return prices
    return {}
