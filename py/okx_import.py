...
# --- Manuelle Bestaende (z.B. Cold-Storage-Wallet) --------------------------
#
# Fuer Bestaende, die NICHT ueber OKX laufen (z.B. eine Trezor-Hardware-
# Wallet), aber trotzdem korrekt in die FIFO-Haltefristen-Berechnung nach
# §23 EStG einfliessen sollen. Eigenes, einfaches CSV-Format (kein OKX-Meta-
# Header wie bei read_okx_csv()) - eine Zeile je einzelnem Kauf, mit dem
# ECHTEN urspruenglichen Kaufdatum/-preis, nicht dem Datum des Transfers auf
# die Wallet. "Herkunft" ist reine Notiz, steuerlich nicht relevant.
#
# Erwartete Spalten: Datum (JJJJ-MM-TT), Asset, Menge, EUR-Kosten, Herkunft

MANUAL_LOTS_COLUMNS = {"Datum", "Asset", "Menge", "EUR-Kosten"}


def read_simple_csv(path):
    """Einfaches CSV mit Kopfzeile in Zeile 1 - fuer selbst erstellte Dateien,
    im Unterschied zu read_okx_csv() ohne vorangestellte OKX-Meta-Zeile.
    Erkennt Komma- und Semikolon-Trennung (Excel im deutschen Gebietsschema
    exportiert oft mit Semikolon)."""
    text = Path(path).read_text(encoding="utf-8-sig")
    ...
    def has_content(row):
        return any((v or "").strip() for v in row.values() if not isinstance(v, list))

    return [r for r in rows if has_content(r)]


def import_manual_lots(conn, rows, source):
    """Legt je Zeile einen eigenen Kauf-Vorgang an (tx_type 'trade_spot',
    Konto 'wallet', Topf 'par23') - bewusst NICHT als ein Sammel-Eintrag fuer
    den Gesamtbestand, sonst rechnet die FIFO-Logik mit einem Durchschnitts-
    datum/-preis statt korrekt pro Kauf-Los. Ist der EUR-Preis fuer eine
    Zeile leer, schaetzt die App die Kostenbasis spaeter ueber den
    historischen Tageskurs (siehe build_lots() in tax.py)."""
    added, skipped, problems = 0, 0, []

    for i, r in enumerate(rows):
        line = i + 2  # Zeile 1 ist die Kopfzeile
        date_str = (r.get("Datum") or "").strip()
        asset = (r.get("Asset") or "").strip().upper()
        herkunft = (r.get("Herkunft") or "").strip()

        try:
            amount = L.D(r.get("Menge"))
        except Exception:
            problems.append(f"Zeile {line}: \"Menge\" nicht lesbar")
            continue
        cost = None
        if (r.get("EUR-Kosten") or "").strip():
            try:
                cost = L.D(r.get("EUR-Kosten"))
            except Exception:
                problems.append(f"Zeile {line}: \"EUR-Kosten\" nicht lesbar")
                continue

        if not date_str or not asset or amount <= 0:
            problems.append(f"Zeile {line}: Datum, Asset oder Menge fehlt/ungueltig")
            continue
        try:
            datetime.strptime(date_str, "%Y-%m-%d")
        except ValueError:
            problems.append(f"Zeile {line}: Datum \"{date_str}\" nicht im Format JJJJ-MM-TT")
            continue

        entries = [L.Entry(asset, amount, "principal")]
        if cost:
            entries.append(L.Entry("EUR", -abs(cost), "principal"))

        note = "Manueller Bestand (externes Wallet)"
        if herkunft:
            note += f" · Herkunft: {herkunft}"

        tx = L.Transaction(
            ts_utc=f"{date_str}T12:00:00Z", tx_type="trade_spot", account="wallet",
            tax_bucket="par23", source=source,
            external_id=f"manual-lot-{date_str}-{asset}-{amount}-{i}",
            note=note, raw=r, entries=entries,
        )
        if L.add_transaction(conn, tx) is not None:
            added += 1
        else:
...
def import_manual_lots_file(conn, path, source=None):
    source = source or "manual_lots_csv"
    path = Path(path)
    ...
    if dupe:
        ...
        conn.commit()
        return {
            "kind": "manual_lots", "duplicate": True,
            "duplicateOfRun": dupe["id"], "duplicateImportedAt": dupe["started_at"],
            "rowsRead": 0, "imported": 0, "skipped": 0, "problems": [],
            "warnings": [
                f"Diese Datei wurde bereits am {dupe['started_at'][:16].replace('T', ' ')} "
                "importiert. Es wurde nichts erneut eingelesen."],
        }

    rows = read_simple_csv(path)
    if not rows or not (MANUAL_LOTS_COLUMNS <= set(rows[0].keys())):
        raise ImportError_(
            "Spalten nicht erkannt. Erwartet werden mindestens: "
            "Datum, Asset, Menge, EUR-Kosten (Herkunft optional).")

    added, skipped, problems = import_manual_lots(conn, rows, source)
    ...
    conn.execute(
        ...
        (L.now_utc(), source, path.name, file_sha256, len(rows), added, skipped,
         json.dumps({"kind": "manual_lots", "problems": problems}, ensure_ascii=False)))
    conn.commit()

    warnings = []
    if problems:
        shown = "; ".join(problems[:5]) + (" …" if len(problems) > 5 else "")
        warnings.append(f"{len(problems)} Zeile(n) übersprungen: {shown}")

    return {
        "kind": "manual_lots", "duplicate": False,
        "rowsRead": len(rows), "imported": added, "skipped": skipped,
